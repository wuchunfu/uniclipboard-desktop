# Phase 33: Fix file sync eventual consistency - Research
...
- `batch_id` exists in the current `file_transfer` schema. Phase 33 preserves this column but does not actively use it for aggregation. Entry-level aggregation is done through `entry_id`. Cleanup or active utilization of `batch_id` is deferred to a future phase.
...
Phase 33 should keep this slice for progress, but pair it with persisted entry-level status from commands and `file-transfer://status-changed`.
