# Phase 32: Fix file sync eventual consistency - Context
...
- Clipboard race detection preserved (Phase 31.1 decision)
...
- Consistent with Phase 29 decision: Blake3 hash mismatch = delete temp file, no retry
...
- Concurrent reception from multiple devices: last-completed transfer overwrites clipboard (Phase 31.1 decision preserved)
...
- Failed entries should use the same visual treatment as stale files from Phase 31.1 (grey + strikethrough pattern)
...
- Phase 31.1 stale file UI pattern (grey + strikethrough) — reuse for failed entries
