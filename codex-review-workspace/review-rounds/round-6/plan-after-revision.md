...
- **Testability**: Two pure internal functions for clean separation of concerns:
  1. `sanitize_xdg_runtime_dir(xdg: Option<&str>) -> Option<PathBuf>` — string-level env var cleaning. Returns `None` for `None`, empty, or whitespace-only values. Returns `Some(PathBuf)` for valid non-empty values.
  2. `resolve_daemon_socket_path_from(base: Option<&Path>) -> PathBuf` — path-level resolution. Accepts cleaned base directory. `None` falls back to `/tmp`. Joins with socket name. Applies length guard; falls back to `/tmp` if over limit.
     The public `resolve_daemon_socket_path()` calls `sanitize_xdg_runtime_dir(env::var("XDG_RUNTIME_DIR").ok().as_deref())` then passes result to `_from`. Tests use these pure functions directly — no global env var mutation, parallel-safe.
