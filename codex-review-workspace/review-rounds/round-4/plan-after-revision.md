...
- **Testability**: Expose a pure internal function `resolve_daemon_socket_path_from(base: Option<&Path>) -> PathBuf` that accepts the base directory as a parameter. The function must treat `None` and `Some("")` (empty path) identically — both fall back to `/tmp`. The public `resolve_daemon_socket_path()` reads `$XDG_RUNTIME_DIR`, filters out empty/whitespace-only values, and calls `_from` with the result. Tests use the `_from` variant directly, avoiding global env var mutation and parallel test pollution.
...
- `test_xdg_runtime_dir_empty`: call `resolve_daemon_socket_path_from(Some(Path::new("")))`, verify fallback to `/tmp` (empty base must not produce relative path).
- `test_xdg_runtime_dir_too_long`: construct a long base with `PathBuf::from("/").join("a".repeat(90))` (produces 92-byte base + 27-byte filename = 119 bytes > 103), verify fallback to `/tmp`.
- `test_socket_path_boundary`: construct base so that `socket_path_byte_len(joined) == 103` exactly (base = `"/" + "x" * (103 - 1 - 26)` = 76 "x"s), verify it succeeds. Then add one more byte to base, verify fallback.
