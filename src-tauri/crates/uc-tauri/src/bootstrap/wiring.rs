...
pub fn start_background_tasks(
    ...
    async_runtime::spawn(async move {
        ...
        // When pairing_bridge is provided, use it instead of the legacy pairing loops.
        // The bridge connects to the daemon WebSocket, subscribes to pairing/peers topics,
        // and translates daemon events to frontend-compatible Tauri events.
        if let Some(mut bridge) = pairing_bridge {
            // Register GUI as discoverable by default when bridge starts.
            if let Err(e) = bridge.set_discoverable(true, Some(300_000)).await {
                warn!(error = %e, "failed to set discoverability on bridge startup");
            }

            // Start the bridge - it handles participant-ready lifecycle internally.
            bridge.start().await;

            // Store bridge handle for cleanup (in a real implementation, we'd store this
            // somewhere for shutdown, but for now we just let it run).
            info!("Started pairing bridge for daemon event subscription");
        } else {
            // Fallback: Use legacy pairing action loop (for non-GUI modes).
            ...
            // Fallback: Use legacy pairing event loop (for non-GUI modes).
            registry
