...
- `test_xdg_runtime_dir_whitespace_only`: call `resolve_daemon_socket_path_from(Some(Path::new("   ")))`, verify fallback to `/tmp` (whitespace-only must be treated as empty).
