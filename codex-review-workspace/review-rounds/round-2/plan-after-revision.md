...
- **Length guard**: After joining, check `path.as_os_str().len() <= 103` (i.e. path bytes + NUL terminator <= 104, the sockaddr_un.sun_path limit). On Unix, use `std::os::unix::ffi::OsStrExt::as_bytes().len()` for explicit byte counting. If the XDG_RUNTIME_DIR-based path exceeds this, log a warning and fall back to `/tmp/uniclipboard-daemon.sock`.
...
- `test_socket_path_length`: call `resolve_daemon_socket_path_from(None)` and assert `path.as_os_str().len() <= 103` (byte length; path + NUL must fit in 104-byte sun_path).
