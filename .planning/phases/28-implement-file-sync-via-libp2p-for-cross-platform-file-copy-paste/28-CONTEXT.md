...
Establish the file sync foundation layer. This phase creates the data structures, port traits, database schema, and settings model that phases 30-31 build upon. Also fixes the critical file classification bug where `file://` URIs are misclassified as `Link` instead of `File`.
...
- Implementation will be in `uc-platform/adapters/` (Phase 30)
...
_Phase: 28 (Foundation) — split from original monolithic Phase 28_
