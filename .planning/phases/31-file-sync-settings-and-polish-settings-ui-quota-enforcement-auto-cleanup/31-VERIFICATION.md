...
gaps:
  - truth: 'FSYNC-POLISH requirement ID exists and is tracked in REQUIREMENTS.md'
    ...
    missing:
      - 'Add FSYNC-POLISH requirement definition to .planning/REQUIREMENTS.md and add a traceability row pointing to Phase 31'
human_verification:
...
# Phase 31: File Sync Settings UI, Auto-Cleanup, and Guards Verification Report
...
**Fix:** Add FSYNC-POLISH to `.planning/REQUIREMENTS.md` with a description covering: file sync settings UI controls, per-device quota enforcement, auto-cleanup of expired files, and error handling polish across the file transfer pipeline. Add a traceability row to the table pointing to Phase 31 with status Complete.
