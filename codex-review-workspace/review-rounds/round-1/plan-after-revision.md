...
- **Length guard**: After joining, assert `path.as_os_str().len() + 1 < 104` (including NUL terminator for sockaddr_un). If the XDG_RUNTIME_DIR-based path exceeds this, log a warning and fall back to `/tmp/uniclipboard-daemon.sock`.
...
- `test_socket_path_length`: assert `path.as_os_str().len() + 1 < 104` (byte length including NUL terminator for SUN_LEN limit).
- `test_socket_path_ends_with_sock`: assert filename is `uniclipboard-daemon.sock`.
- `test_xdg_runtime_dir_override`: set `XDG_RUNTIME_DIR` env var to a short path, verify it is used (use `temp_env` or `std::env::set_var` in serial test).
- `test_xdg_runtime_dir_too_long`: set `XDG_RUNTIME_DIR` to a path that would exceed 104 bytes after joining, verify fallback to `/tmp` is used.
- `test_socket_path_boundary`: set `XDG_RUNTIME_DIR` to a path that produces exactly 103 bytes (limit - 1), verify it succeeds.
